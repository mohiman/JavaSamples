 ...
 *
 * Modified by David Antosh for IE8 compatibility.
 * 2015-08-11
 */
    ...
    return {
      ...
      'link': function linkingFunction($scope, element, attr) {
        ...
        var selector = attr.selector
          ...
          , activatorSelector = attr.activateOn
          , activator = activatorSelector ? angular.element(element[0].querySelector('.' + activatorSelector)) : null
          , theCalendar
          ...
          '{{month}} <a href="javascript:void(0)" ng-click="showYearsPagination = !showYearsPagination"><span>{{year}}</span> </a>' +
          '</div>' +
        ...
        angular.forEach(['0', '1', '2', '3', '4', '5', '6'], function mappingFunc(el) {
          $scope.daysInString.push($filter('date')(new Date(new Date('06/08/2014').valueOf() + A_DAY_IN_MILLISECONDS * el), 'EEE'));
        });
        ...
        angular.element(document).bind('click focus', function onClickOnWindow() {
          if (!isMouseOn &&
        ...
        $scope.setInputValue = function manageInputValue() {
          ...
          if ($scope.isSelectableMinDate($scope.year + '/' + $scope.monthNumber + '/' + $scope.day)
            ...
            $timeout(function() { thisInput.change(); });
          } else {
        ...
        $scope.showCalendar = function manageShowCalendar() {
          ...
          angular.element('._720kb-datepicker-calendar').removeClass('_720kb-datepicker-open');
